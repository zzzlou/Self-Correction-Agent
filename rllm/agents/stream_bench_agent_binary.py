...
class StreamBenchAgentBinary(BaseAgent):
    ...
    def update_from_model(self, response: str, **kwargs) -> Action:
        ...
        # 2. 【关键】如果输出了 learn，立即存入 Agent 的内存
        if learn_content:
            # 简单去重，避免重复添加相同的规则
            if learn_content not in self.learned_rules:
                self.learned_rules.append(learn_content)
    ...
    def update_from_env(self, observation: Any, reward: float, done: bool, info: dict, **kwargs):
        ...
        if not done:
            ...
            # --- A. 构造 Rules 部分 (从 Agent 内存读取) ---
            rules_section = "No rules learned yet."
            if self.learned_rules:
                rules_list = "\n".join([f"{i+1}. {r}" for i, r in enumerate(self.learned_rules)])
                rules_section = rules_list
            
            # --- B. 构造 Feedback 部分 (从 Env info 读取) ---
            # 只有从第二题开始才有反馈
            feedback_str = "None. This is the first question."
            if self._trajectory.steps: # 如果有过历史步骤
                # 从 info 中提取上一题的上下文
                # 我们要求 Env 在 info 里必须传回：'previous_question', 'is_correct', 'ground_truth'
                is_correct = info.get('is_correct', False)
                status = "✅ Correct" if is_correct else "❌ Incorrect"
                
                prev_q = info.get('previous_question_text', 'Unknown')
                prev_ans = info.get('previous_model_answer', 'Unknown')
                # ground_truth = info.get('previous_ground_truth', 'Unknown')
                
                feedback_str = (
                    f"- Previous Question: {prev_q}\n"
                    f"- Your Answer: {prev_ans}\n"
                    f"- Result: {status}\n"
